...
_n = ati([0, 7, 12, 14]) # ati函数将数组中的每一个元素转换为整形，方便您可以使用1E4一类的数来表示数据大小
_m = ati([0, 6, 24, 24]) 
...
for i in range(1, 4): # 即在[1, 4)范围内循环，也就是从1到3
    ...
    # graph = Graph.graph(n, m) # 生成一个n点，m边的随机图，边权限制为5
    ...
    test_data.output_gen("..exe") # 标程编译后的可执行文件，不需要freopen等，CYaRon自动给该程序输入并获得输出作为.out